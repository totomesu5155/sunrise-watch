...
MAX_ATTEMPTS = max(1, int(os.getenv("SUNRISE_MAX_ATTEMPTS", "4")))
RETRY_BACKOFF_SECONDS = (15, 25, 40)
MAX_RUN_SECONDS = 210
...
def run_once() -> int:
    ...
    with sync_playwright() as pw:
        ...
        # 手作りUAではなくPlaywright公式のSamsung端末プロファイルを使う。
        # user-agentだけでなく is_mobile / has_touch / deviceScaleFactor も
        # 一貫した値になるため、e5489の端末判定との食い違いを減らす。
        device = pw.devices["Galaxy S24"]
...
def open_later_trains(page) -> None:
    """
    「この列車を変更」ポップアップを開いたあと、
    「後の列車」ボタンの通常submitには頼らず、
    現在のe5489セッションを保ったまま CBChangeTrainSP へPOSTする。

    添付された実ページでは:
      - 「後の列車」button は formMain1 内の type=submit
      - formMain1 のactionは CBVacantCompSP
      - 手動操作で到達する「列車の変更」ページは CBChangeTrainSP

    Headless環境ではサイト側JSのsubmit切替がうまく働かず、
    CBChangeTrainSPで「ご案内」になることがあるため、
    ブラウザ内で一時formを作って正規POST遷移させる。
    """
    change = page.get_by_text(
        "この列車を変更",
        exact=True,
    ...
    # 実HTML上、後の列車buttonはformMain1のsubmit。
    # ただしHeadlessでは既定action(CBVacantCompSP)側へ流れて
    # その後CBChangeTrainSPで「ご案内」になることがある。
    # そこで現セッションのdocumentから明示的にPOSTする。
    print(
        "  -> 「後の列車」: CBChangeTrainSP へ明示POST",
        flush=True,
    )

    target = (
        "https://e5489.jr-odekake.net"
        "/e5489/cssp/CBChangeTrainSP"
    )
    ...
    try:
        ...
        # navigationイベント取り逃しに備えて実ページで判定
        page.wait_for_timeout(1200)

    page.wait_for_timeout(800)

    if is_guide_or_error(page):
        # 公開ログには旅行条件等を出さず、エラーコードだけ拾う
        try:
            body = norm(
                page.locator("body").inner_text(
                    timeout=5000
